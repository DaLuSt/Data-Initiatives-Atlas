...
alternative_names:
  ...
  - Kıbrıs
description: >
  Country anchor entity for Cyprus, a member state of the European Union
...
relationships:
  - type: part-of
    target: EU
    source: fact
    evidence: "Cyprus is one of the 27 member states of the European Union, having acceded on 1 May 2004; the Union's own list of EU countries records its accession date together with its Schengen and euro status (european-union.europa.eu 'EU countries'). NOT READ — search-only. Anchor edge under metadata/relationship-types.md §2.3: it records EU membership and asserts no more than that."
    confidence: medium
    valid_from: null
    valid_until: null

sources:
  - title: "CY — Cyprus (ISO 3166-1 country code)"
    url: "https://www.iso.org/obp/ui/#iso:code:3166:CY"
...
Cyprus (ISO 3166-1 alpha-2: **`CY`**) is a **base country anchor**,
created so that entities scoped to it have somewhere to attach. No Cyprus
...
| Schengen area | No |
| Council of Europe | Member since 1961 |
...
## An EU member state outside Schengen, and an acquis that stops at a line

Cyprus is one of **two EU member states outside the Schengen
area**, with [[IE]] — and the two are outside for opposite reasons. Ireland
holds an opt-out; Cyprus does not, and is committed to joining.

The island is divided, and the *acquis communautaire* is **suspended** in
the areas not under the effective control of the Republic's government. That
is a shape the Atlas cannot express: `applies-in` is a whole-country
predicate, so an EU instrument either applies in Cyprus or it does not.
...
No EU instrument in the Atlas carries `applies-in` → [[CY]] yet.
