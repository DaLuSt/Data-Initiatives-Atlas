...
alternative_names:
  ...
  - Republika Hrvatska
description: >
  Country anchor entity for Croatia, a member state of the European Union
  since 1 July 2013. It is a base anchor: it carries the country's
...
relationships:
  - type: part-of
    target: EU
    source: fact
    evidence: "Croatia is one of the 27 member states of the European Union, having acceded on 1 July 2013; the Union's own list of EU countries records its accession date together with its Schengen and euro status (european-union.europa.eu 'EU countries'). NOT READ — search-only. Anchor edge under metadata/relationship-types.md §2.3: it records EU membership and asserts no more than that."
    confidence: medium
    valid_from: null
    valid_until: null

sources:
  - title: "HR — Croatia (ISO 3166-1 country code)"
    url: "https://www.iso.org/obp/ui/#iso:code:3166:HR"
...
Croatia (ISO 3166-1 alpha-2: **`HR`**) is a **base country anchor**,
created so that entities scoped to it have somewhere to attach. No Croatia
...
| Framework | Status |
|---|---|
| European Union | Member state since **1 July 2013** |
| Euro area | Since **1 January 2023** |
| Schengen area | Full member since **1 January 2023** |
| Council of Europe | Member since 1996 |
...
Croatia is the **newest EU member state**, and the only one to
have joined since the Atlas's oldest modelled instruments were written.

It is also the only member state to have entered the euro area and the
Schengen area **on the same day**, 1 January 2023 — a combination no other
accession has managed.
...
No EU instrument in the Atlas carries `applies-in` → [[HR]] yet.
