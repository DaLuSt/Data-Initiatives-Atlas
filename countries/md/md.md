...
alternative_names:
  ...
  - Republica Moldova
description: >
  Country anchor entity for Moldova, a candidate country for European
...
relationships:
  - type: part-of
    target: INTL-COE
    source: fact
    evidence: "Confirmed by reading two Wikipedia articles directly (2026-08-27) after coe.int itself proved genuinely, domain-wide bot-walled (403) on every path tried: the 'Member states of the Council of Europe' article's own accession table gives Moldova's accession date as 13 July 1995, the same day as Albania; the general 'Council of Europe' article corroborates current membership. ISO's OBP is also confirmed blocked (403) — it is a JavaScript application, not a static page. The EU-candidacy year (2022) rests on enlargement.ec.europa.eu, read directly in the prior pass but not independently re-confirmed this pass to a specific month/day. Anchor edge under metadata/relationship-types.md §2.3: it records Council of Europe membership and asserts no more than that."
    confidence: high
    valid_from: 1995-07-13
    valid_until: null

sources:
  - title: "MD — Moldova (ISO 3166-1 country code)"
    url: "https://www.iso.org/obp/ui/#iso:code:3166:MD"
...
Moldova (ISO 3166-1 alpha-2: **`MD`**) is a **base country anchor**,
created so that entities scoped to it have somewhere to attach. No Moldova
...
Moldova was granted EU candidate status on the **same day as
[[UA]]** in June 2022, in the aftermath of the full-scale invasion of
Ukraine, and completed the screening of its alignment with EU law in 2025.

Moldova and Ukraine, granted status in 2022, are now reported to be ahead of
[[RS]] and [[MK]], which have been negotiating far longer — the clearest
recent evidence that time in the queue and progress through it are different
things.
