...
| [[FR-LRN]] → [[EU-OPEN-DATA-DIRECTIVE]] | **Resolved as a deliberate negative, not a gap.** [[EU-OPEN-DATA-DIRECTIVE]]'s own "France is a third case, not a missing one" section and [[FR-LOI-VALTER]]'s "documented negative" (the Commission's infringement letter names 19 member states over this directive; France is absent) already concluded that France passed no ODD-era instrument because its 2015/2016 regime already satisfied it — no `implements-requirement-from` edge is asserted from France on that basis, deliberately. The "2021 ordinance" belief is also corrected there: that ordinance (2021-1518) transposes a copyright directive, unrelated to open data. | **Closed — deliberate negative, already resolved** |
| [[FR-FRANCECONNECT]] → [[EU-EIDAS]] / [[EU-EIDAS2]] | Nothing read mentions eIDAS or cross-border recognition, even though a national identity federation is precisely what eIDAS governs. The eIDAS2 wallet deadline (end 2026) is now four months away and **no country in the Atlas is linked to it**. | **Closed 2026-09-05** — the eIDAS obligation belongs to the identity scheme, not the federation broker. [[FR-FRANCE-IDENTITE]], split out as its own entity, carries a high-confidence `implements-requirement-from` → [[EU-EIDAS]] (a direct Commission notification, 9 Sept 2024) and a low-confidence one to [[EU-EUDI-WALLET]] (a Commission designation as France's future wallet, not yet operational). FranceConnect itself correctly carries neither. |
...
| Belgium, France, Spain | **Open Data Directive transpositions still unidentified.** | All three amended existing law rather than passing a standalone act, which is why they are harder to find than Ireland's S.I. | 2026-08-18 | **Closed** — Belgium via [[BE-HERGEBRUIK-WET-2023]] and Spain via [[ES-RDL-24-2021]] amending [[ES-LEY-37-2007]] both carry `implements-requirement-from`, confirmed; France deliberately carries none — see [[FR-LOI-VALTER]]'s "documented negative" and [[EU-OPEN-DATA-DIRECTIVE]]'s "France is a third case, not a missing one" section. |
