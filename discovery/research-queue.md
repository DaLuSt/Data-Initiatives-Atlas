...
| Government namespaces without an apex site | `gob.es` is one; `gov.cz`, `gov.pt` and `public.lu` are also namespaces in the Atlas's citations and were **not** in the checked top nineteen, so whether their apexes serve a site is unknown. The `Example host` column makes the report safe either way, but a generated apex check would make it certain | Later | Manual link check / 2026-08-20 |
