...
| ~~Committee of Convention 108 (T-PD)~~ **— DONE 2026-08-20** ([[INTL-TPD]]) | The Convention's consultative committee — its counterpart to [[EU-EDPB]]. Created `coverage: low`: `coe.int` is blocked, so its composition, mandate and cadence are all unestablished | **Next** | Convention 108 batch / 2026-08-19 |
...
| Statistikaamet (Statistics Estonia) | Would be the twelfth national statistical office and another [[EU-ESS]] member. Named only as a research-project partner in what was read | Next | Estonia batch / 2026-08-20 |
...
| Narrow the `gob.es` link failure | A human check on 2026-08-20 found `gob.es` inaccurate while the other eighteen highest-value domains were fine. Three plain-`http://` `espanadigital.gob.es` citations are fixed; the other 30 URLs across 14 subdomains cannot be narrowed from inside a session because the proxy blocks the host. Needs either the allowlist or a second manual pass naming the specific bad URLs | **Next** | Manual link check / 2026-08-20 |
