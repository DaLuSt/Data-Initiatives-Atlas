---
id: FR-LOI-RENSEIGNEMENT-2015
type: law
name: Loi n° 2015-912 du 24 juillet 2015 relative au renseignement
alternative_names:
  - Loi renseignement
  - Loi du 24 juillet 2015
  - French Intelligence Act 2015
  - Code de la sécurité intérieure, Livre VIII
description: >
  French act of 24 July 2015 on intelligence, codified in Book VIII of the
  Code de la sécurité intérieure. It defines the intelligence-gathering
  techniques that may be used, the purposes that may justify them and the
  authorisation procedure — requests go to the Prime Minister, who decides
  after an opinion from the CNCTR — and opens a right of appeal to the
  Conseil d'État.

level: national
country: FR
region: EU

status: active
confidence: medium
coverage: medium
verification: primary-source

start_date: 2015-07-24
end_date: null
last_verified: "2026-08-26"
previous_version: null
successor: null

domains:
  - DOMAIN-NATIONAL-SECURITY
organisations: []
related_entities:
  - FR-DGSE
  - FR-DGSI
  - FR-DRM
  - FR-DRSD
  - FR-CNCTR
relationships: []

sources:
  - title: "10 ans de la loi du 24 juillet 2015 relative au renseignement"
    url: "https://www.dgse.gouv.fr/fr/la-dgse/nos-actualites/10-ans-de-la-loi-du-24-juillet-2015-relative-au-renseignement"
    publisher: "Direction générale de la Sécurité extérieure (DGSE)"
    accessed: "2026-08-26"
  - title: "Les techniques de renseignement contrôlées par la CNCTR"
    url: "https://www.cnctr.fr/en/techniques-de-renseignement"
    publisher: "Commission nationale de contrôle des techniques de renseignement (CNCTR)"
    accessed: "2026-08-26"
  - title: "Les finalités pouvant légalement justifier le recours à des techniques de renseignement"
    url: "https://www.cnctr.fr/en/finalites"
    publisher: "Commission nationale de contrôle des techniques de renseignement (CNCTR)"
    accessed: "2026-08-26"
  - title: "LOI n° 2015-912 du 24 juillet 2015 relative au renseignement"
    url: "https://www.legifrance.gouv.fr/jorf/id/JORFTEXT000030931899"
    publisher: "Légifrance / Direction de l'information légale et administrative"
  - title: "LOI n° 2015-912 du 24 juillet 2015 relative au renseignement — dossier législatif"
    url: "https://www.legifrance.gouv.fr/dossierlegislatif/JORFDOLE000030375694/"
    publisher: "Légifrance / Direction de l'information légale et administrative"
---

# Loi du 24 juillet 2015 relative au renseignement

> **Verified 2026-08-26.** DGSE's own tenth-anniversary page and both
> CNCTR pages were read directly. The codification is more precise
> than this entity previously recorded: the purposes sit at Article
> L.811-3 and the techniques at Title V of Book VIII, not just "Book
> VIII" generally. Both Légifrance citations remain genuinely
> bot-walled (403) even with an honest User-Agent.

## Description

The 2015 intelligence law, codified as **Book VIII of the Code de la
sécurité intérieure** — its purposes at **Article L.811-3** and its
techniques at **Title V** of that book, confirmed by reading the
DGSE's own tenth-anniversary page directly — is the single instrument
governing all four French services in the Atlas. It legislates three
things:

1. **Which techniques** may be used — metadata access, categories of
   interception, and more intrusive devices capturing words and images in a
   private place or collecting computer data.
2. **Which purposes** may legally justify their use.
3. **Who authorises them** — the Prime Minister, after an opinion from
   [[FR-CNCTR]].

## A law about techniques, not about services

This is the structural fact that shapes the whole French cluster. The act
does not constitute [[FR-DGSE]], [[FR-DGSI]], [[FR-DRM]] or [[FR-DRSD]];
it defines a catalogue of powers and designates who may reach into it.

The consequence for the graph is that all four services point at this one
entity. Germany's cluster is three parallel pairs; France's is a star.

[[GB-IPA-2016]] makes the same choice in the UK, and [[DE-G10]] does it for
one specific right in Germany.

## Codification: this entity is both the act and Book VIII

The Atlas holds **one** entity for the *loi* and its codified form. French
law routinely dissolves an act into a code, and modelling the two separately
would create a duplicate with no way to keep them consistent.

The frontmatter records "Code de la sécurité intérieure, Livre VIII" as an
alternative name so that a search for either resolves here. This follows the
treatment already given to [[FR-LIL]], which the Atlas holds as one entity
across its 1978 original and its 2018 GDPR-conformity rewrite.

## Appeal to the Conseil d'État

The act opens a right of appeal to the **Conseil d'État**, available to any
citizen with an interest in acting. [[FR-CNCTR]] may also refer a matter
there when it believes an authorisation was granted irregularly. The court
may annul the decision, compensate the applicant, **order the destruction of
collected data**, and refer the matter to the public prosecutor.

## Not modelled

- The **law of 30 July 2021** on the prevention of terrorist acts and on
  intelligence, which amended this regime.
- The **decree of 12 May 2014, revised 14 June 2017**, defining the
  intelligence community, and the **Code de la défense** provisions the
  sources say now inscribe it.
- The **second circle** of services, which the sources say may also access
  intelligence techniques.
- The **Conseil d'État** and the **délégation parlementaire au renseignement**.

## Sources

Listed in frontmatter, three read directly this pass. Both Légifrance
citations remain genuinely bot-walled (403) even with an honest,
identifying User-Agent — DGSE's own page and CNCTR's two pages now
carry the load these were meant to.
