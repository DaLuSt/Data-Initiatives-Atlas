---
id: GB-JSA-2013
type: law
name: Justice and Security Act 2013
alternative_names:
  - JSA 2013
description: >
  United Kingdom act which, in Part 1, re-founded the Intelligence and
  Security Committee of Parliament as a statutory committee and provided for
  oversight of the Security Service, the Secret Intelligence Service, GCHQ
  and other government activities relating to intelligence or security
  matters, extending the committee's remit to operations.

level: national
country: GB
region: null

status: active
confidence: medium
coverage: medium
verification: search-only

start_date: 2013-04-25
end_date: null
last_verified: "2026-08-18"
previous_version: null
successor: null

domains:
  - DOMAIN-NATIONAL-SECURITY
organisations: []
related_entities:
  - GB-ISC
  - GB-MI5
  - GB-SIS
  - GB-GCHQ
  - GB-ISA-1994
relationships:
  - type: references
    target: GB-ISA-1994
    source: fact
    evidence: "The Intelligence Services Act 1994 established the Intelligence and Security Committee, a body consisting of members of each House of Parliament, with the function of examining the expenditure, administration and policy of the Agencies; the Intelligence and Security Committee of Parliament is now a statutory Committee under Part 1 of the Justice and Security Act 2013, with responsibility for oversight of the expenditure, administration, policy and operations of the intelligence services (legislation.gov.uk ukpga/2013/18 explanatory notes; en.wikipedia.org 'Justice and Security Act 2013'). NOT READ — search-only."
    confidence: medium
    valid_from: 2013-04-25
    valid_until: null

sources:
  - title: "Justice and Security Act 2013 — explanatory notes"
    url: "https://www.legislation.gov.uk/ukpga/2013/18/notes/division/3/2/data.htm"
    publisher: "The National Archives (legislation.gov.uk)"
  - title: "Justice and Security Act 2013"
    url: "https://en.wikipedia.org/wiki/Justice_and_Security_Act_2013"
    publisher: "Wikipedia"
  - title: "Oversight and where we fit in"
    url: "https://investigatorypowerstribunal.org.uk/oversight-and-where-we-fit-in/"
    publisher: "Investigatory Powers Tribunal"
---

# Justice and Security Act 2013

> **Sourcing caveat.** Compiled from search-engine results only; the cited
> pages were confirmed to exist but were not read. `verification:
> search-only`.

## Description

Part 1 of the 2013 act re-founded [[GB-ISC]] as a statutory committee and
provides for oversight of [[GB-MI5]], [[GB-SIS]], [[GB-GCHQ]] and other
government activities relating to intelligence or security matters.

## One word wider

[[GB-ISA-1994]] gave the ISC the function of examining the agencies'
**expenditure, administration and policy**. This act gives it expenditure,
administration, policy **and operations**.

A committee limited to the first three scrutinises how an agency is *run*.
Adding *operations* lets it scrutinise what the agency *does*. It is the
single most consequential difference between the 1994 and 2013 arrangements,
and it is one word.

## `references`, not `supersedes`

The relationship to [[GB-ISA-1994]] is `references`. The 1994 act is still
in force and still constitutes [[GB-SIS]] and [[GB-GCHQ]]; only its
oversight provision was displaced.

`supersedes` in this repository asserts that one instrument has replaced
another, and carries the expectation that the target takes
`status: superseded`. That would be wrong here — and the same reasoning is
recorded on [[GB-ISA-1994]] and on [[GB-IPA-2016]], which amends the 1994
act through Schedule 10.

Three UK acts touching one another without any of them retiring another is
unusual enough in the Atlas to be worth stating three times.

## Not modelled

- The act's **Part 2**, on closed material procedures in civil proceedings —
  the more litigated half of the statute, and unrelated to the Atlas's
  subject matter.
- The ISC's **memorandum of understanding** with the Prime Minister, which
  in practice defines the operational remit.

## Sources

Listed in frontmatter.
