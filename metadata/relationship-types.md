...
| `supersedes` | This entity has formally replaced another (see also `status: superseded` + `successor` on the old entity). **Ontology decision, 2026-09-20** (closing `discovery/unresolved.md` item #34): does a Bund-Länder-style institutional transformation need a second type distinguishing "succession" (legal continuity, a `następca prawny`/legal-successor relationship) from "abolition-and-replacement" (the old body dissolved, a fresh one created, functions redistributed with no continuing legal personality)? No — every case examined so far ([[ES-AEAD]]'s own file, modelled `supersedes` [[ES-SGAD]]; [[PL-UODO]]'s own 2018 annual report describing itself as GIODO's `następca prawny`; Poland's proposed COI→Agencja Informatyzacji transformation) is, on its own source's language, a case of continuity, not abolition-with-no-continuity — so no modelled or candidate case actually needs the distinction to be a first-class graph edge rather than a fact narrated in the entity's own evidence prose, exactly as [[ES-AEAD]] already does. Revisit only if a genuine abolition-and-replacement case is found — one whose own source explicitly denies legal continuity to the successor, not merely one this Atlas hasn't yet modelled as an entity (GIODO and Agencja Informatyzacji remain unmodelled for want of a two-sided source, an unrelated verification gap, not a vocabulary one) |
...
| `amends` | This instrument modifies the text of another instrument, which continues to exist under its own name and date. Distinct from `supersedes`, which retires the target, and from `implements-requirement-from`, which records the EU obligation being discharged. An amending act typically carries both: `amends` the domestic act it edits, `implements-requirement-from` the directive it transposes. Added in the third research-queue batch, where three of the five Open Data Directive transpositions turned out to be amendments to pre-existing national re-use acts rather than standalone instruments. **Applies regardless of how extensive the revision is** — [[DE-NIS2UMSUCG]]'s edge to [[DE-BSIG]] was recorded `supersedes` at `confidence: low` in the German batch (2026-08-15) for want of a better type, because "comprehensively revised" felt too strong for `amends`' first examples; re-examined 2026-09-20 as an ontology decision closing `discovery/unresolved.md` items #7 and #68, `amends` fits without qualification — the type's own definition turns on whether the target *continues to exist under its own name*, not on how much of its text changed. A future amending act, however sweeping, should be checked against `amends` before a weaker or overstating type is used |
...
| `supplements` | A national instrument fills in optional/discretionary provisions (opening clauses, derogations) that a higher-level instrument leaves to national law, where that higher-level instrument is already directly applicable and not itself being transposed. Distinct from `implements-requirement-from`, which records a transposition obligation being discharged — a `supplements` target is already in force in the jurisdiction without the national instrument's help. Added 2026-08-21 for [[LI-DSG]], which exercises the GDPR's national opening clauses (employment data, journalistic expression, video surveillance, national identification numbers, criminal-conviction data) after the GDPR became directly applicable in Liechtenstein through the EEA Agreement — the prior text on that entity used `implements-requirement-from` as the closest available type while explicitly flagging that it overstated the relationship |
| `uses-data-from` | An organisation is an authorised consumer ("afnemer") of another entity's data for its own operational purpose, without holding, maintaining or governing that entity. Distinct from `depends-on`, which would overstate operational necessity, and from `applies-to`, which would invert the direction. Added 2026-09-20 for the Dutch stelsel van basisregistraties, whose own documentation describes "provider, holder and user" as three distinct roles an organisation can occupy toward the same register — the Atlas could record only the first two (via `maintained-by`) until this type existed. First examples: [[NL-BELASTINGDIENST]] (uses [[NL-WOZ]] property values for several national taxes) and [[NL-RDW]] (uses [[NL-BRP]] data, per the stelsel's own worked example of an organisation being provider, holder and user of the vehicle register at once) |
| `carries-identifier-of` | This entity's own records incorporate another entity's identifier as a shared key, coupling the two datasets without either governing, maintaining or depending operationally on the other. Distinct from `derived-from`, which would imply the whole record was produced by adapting the other, and from `references`, which suggests a document citation rather than a data-level key. Added 2026-09-20 for the Dutch stelsel van basisregistraties. First examples: [[NL-BRK]] (its own products increasingly carry the KvK number, [[NL-NHR]]'s identifier, for organisations) and [[NL-BRP]] (since January 2024, every current residential/postal address must carry a BAG identification code from [[NL-BAG]] and match its values exactly) |
| `referred-to-court-over` | A subject (typically a member state's national transposing instrument) was referred by an enforcement authority to a court, for failure to comply with or transpose the target instrument. Deliberately generic on **which** court: the specific court is named in `evidence` and cross-referenced via `related_entities`, the same way `governed-by`'s specific governing body or `maintained-by`'s specific maintainer is named in `target`/`evidence` rather than folded into the type — a type name should describe the kind of relationship, not the specific party on the other end of it. All current examples are Court of Justice of the EU referrals (TFEU Articles 258/260 infringement procedure) via [[EU-CJEU]], but the type is written to also cover, without renaming, a future case involving a different court (a constitutional court, the ECtHR). Distinct from `implements-requirement-from`, which records the transposition obligation itself, from `applies-in`, which records mere applicability, and from a plain `related-to`, which would lose the "this is specifically a judicial referral" content — one procedural stage short of a judgment. Recorded on the national instrument rather than on the court entity, matching the phrasing "referred to the Court over [instrument]" this gap was originally flagged under on [[EU-OPEN-DATA-DIRECTIVE]] and [[EU-CJEU]]'s own entities. Where no national transposing instrument exists as an Atlas entity (e.g. Bulgaria and Latvia's Open Data Directive referral), the fact is recorded in the directive's own prose instead — a scoping gap, not a vocabulary one. Added 2026-09-20 (originally as the CJEU-specific `referred-to-cjeu-over`, renamed the same day before any release for consistency with the rest of this vocabulary — see git history). First examples: [[IE-NCS-BILL]], [[ES-LCGC]], [[FR-NIS2-LOI]] and [[NL-CBW]] (all referred to [[EU-CJEU]] over [[EU-NIS2]] on 8 July 2026) and [[BE-HERGEBRUIK-WET-2023]] and [[NL-WHO]] (referred to [[EU-CJEU]] over [[EU-OPEN-DATA-DIRECTIVE]] on 15 February 2023). This does **not** close the separate, larger ontology gap of having no entity type for an individual infringement procedure (the stages before and after the referral, and any resulting judgment, remain unmodelled) |
