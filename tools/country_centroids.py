"""Approximate country/region centroids for the site's geographic map layout.

Country coordinates are average-centroid points (not capitals, and not any
other single reference point), sourced from the public-domain "countries.csv"
published in Google's DSPL sample data set:
https://github.com/google/dspl/blob/master/samples/google/canonical/countries.csv
(fetched 2026-09-17). A country's centroid can sit somewhere unintuitive for
its shape — Russia's is deep in Siberia, not near Moscow — since it is a
geometric average, not a claim about where anything important happens.

Region centroids have no equivalent public dataset (a "region" here is a
political grouping, not a landmass) and are chosen by hand, one entry at a
time, with the reasoning recorded alongside each.

Deliberately comprehensive rather than limited to the ~60 codes currently in
use: the Atlas discovers countries from the data as they are added (brief
§"facets, discovered from the data, never hard-coded"), and a newly added
country should not silently break the map view or need a companion PR here.
build_graph.py fails the build instead if a country or region ever appears
in the data without an entry in one of these two tables — see
tools/test_build_graph.py's TestGeographicData.
"""

from __future__ import annotations

# ISO 3166-1 alpha-2 code -> (latitude, longitude).
...
# Region code (metadata/ontology.md §2.1) -> (latitude, longitude). Unlike
# countries, there is no ready-made dataset to draw from — a region is a
# political grouping the Atlas itself defines, not a landmass with a
# geometric centroid. Each entry is chosen and justified individually.
REGION_CENTROIDS: dict[str, tuple[float, float]] = {
    # Brussels — seat of the European Commission, the Council and the
    # Parliament's committee work. Not a claim that EU law is "centred"
    # there, just a stable, defensible point to anchor EU-scoped entities
    # on the map.
    "EU": (50.8503, 4.3517),
}
